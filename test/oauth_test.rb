...
class OauthTest < Minitest::Test
  ...
  ERROR_CLASSES = {
    'oauth' => InternetData::OauthRequestError,
  ...
  def test_only_the_idempotent_operations_retry
    OAUTH['retries']['cases'].each do |c|
      ...
      api = InternetData::Client.new(base_url: @origin.base_url, timeout: 2).oauth
  ...
  def test_poll_device_token_follows_every_corpus_case
    OAUTH['poll']['cases'].each do |c|
      ...
      device = InternetData::DeviceAuthorization.build_from_hash(c['device'])
  ...
  def test_no_oauth_request_carries_the_api_key
    ...
    client = InternetData::Client.new(api_key: rule['apiKey'], base_url: @origin.base_url, timeout: 2)
    api = client.oauth
  ...
  def test_a_2xx_that_is_not_its_type_is_an_ordinary_server_error
    ...
    answers.each do |answer|
      ...
      assert_instance_of InternetData::Error, error, answer.inspect
  ...
  def test_an_answer_missing_any_one_required_member_is_an_ordinary_server_error
    ...
    calls = {
      InternetData::OauthMetadata => ->(api) { api.metadata },
    ...
    required.each do |type, members|
      members.each do |member|
        ...
        api = InternetData::Client.new(base_url: @origin.base_url, timeout: 2, retries: 0).oauth

        kind, error = bounded { calls.fetch(type).call(api) }

        assert_equal :error, kind, "#{type} without #{member}: #{error.inspect}"
        assert_instance_of InternetData::Error, error, "#{type} without #{member}"
  ...
  def test_every_oauth_method_takes_a_per_call_timeout
    server = Origin.new(body: '', json_pace: { stall: 8 })
    api = InternetData::Client.new(base_url: server.base_url, timeout: 4, retries: 0).oauth
    FakeClock.new.install(api)
    device = InternetData::DeviceAuthorization.build_from_hash(OAUTH['poll']['cases'].first['device'])
    calls = {
  ...
  def keyless(responses)
    ...
    InternetData::Client.new(base_url: @origin.base_url, timeout: 2)
  end
  ...
  def assert_error_type(type, error, name)
    assert_kind_of InternetData::Error, error, name
    if type == 'client'
      refute_kind_of InternetData::OauthRequestError, error, "#{name}: an ordinary failure is not a refusal"
    else
